			const pics = document.querySelectorAll(".pic");  // pic인 요소들을 가져와 pics 라는 변수에 저장
			const lightbox = document.querySelector("#lightbox");  // document.getElementById("lightbox"); 도 가능
			const lightboxImage = document.querySelector("#lightboxImage");	 // document.getElementById("lightboxImage"); 도 가능		

			for(let i=0; i < pics.length; i++) {  // pics 배열에 있는 모든 이미지에서 반복
