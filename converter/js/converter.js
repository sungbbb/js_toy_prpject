		const sUnit = document.querySelector("#s-unit");  // 왼쪽 텍스트 필드 옆에 표시하는 단위
		const tUnit = document.querySelector("#t-unit");  // 오른쪽 텍스트 필드 옆에 표시하는 단위
		const cToF = true;  // 섭씨에서 화씨로 변환
		
		const source = document.querySelector("#s-value");  // 왼쪽 필드
		const target = document.querySelector("#t-value");   // 오른쪽 필드
